...
\usepackage{bm}
\usepackage{bbm}
\usepackage{mathtools,amsthm,amssymb, nccmath}
...
\begin{document}
	...
	\begin{enumerate}[wide = 0pt, label = \textbf{Problem \arabic*:}]
		\item {Let $ x_0 $ be deterministic and $ x_1 , \dots , x_N $ denote random variables satisfying (an autoregressive model of order 1) $ x_n = \alpha x_{n−1} + w_n $ , $ n = 1, . . . , N $, where $ w_1 , . . . , w_N $ are independent and identically distributed Gaussian random variables with mean 0 and variance 1 while $ \alpha $ denotes an unknown parameter. }
		\begin{subquestion}
			\item {Find the joint density of $ x_1 , . . . , x_N $ given $ \alpha $ (remember x 0 is deterministic).}
